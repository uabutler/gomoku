...
# The most important piece of member data is board. It stores the
...
class Board:
    ...
    # return empty if nobody won, or the peice of the respective winner

    def checkWin(self):
        def checkSquare():
            nonlocal previous, current, i, j
            if self.board[i][j] != square.empty and self.board[i][j] == previous:
                current += 1
            else:
                previous = self.board[i][j]
                current = 0 if previous == square.empty else 1
            if current == 5:
                return board.board[i][j]
            else:
                return square.empty
        for i in range(self.size):
            previous = self.board[i][0]
            current = 0 if previous == square.empty else 1
            for j in range(1, self.size):
                if (tmp := checkSquare()) != square.empty:
                    return tmp
        for j in range(self.size):
            previous = self.board[0][j]
            current = 0 if previous == square.empty else 1
...
current = square.x_piece

while(board.checkWin() == square.empty):
    board.getUserMove(current)
    current = square.x_piece if current == square.o_piece else square.o_piece
